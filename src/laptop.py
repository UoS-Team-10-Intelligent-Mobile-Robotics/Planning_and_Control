...
from math_feeg6043 import l2m, Inverse, HomogeneousTransformation
...
class LaptopPilot:
    def __init__(self, simulation):
        ...
        #This part is for the simulation
        ...
        self.northings_path = [0.375, 1.625, 1.625, 0.375, 0.375, 1.625, 1.625, 0.375, 0.375]
        self.eastings_path = [0.375, 0.375, 1.625, 1.625, 0.375, 0.375, 1.625, 1.625, 0.375]
        ...
        self.initialise_pose = True # False once control gains are initialized
        ...
        lidar_xb = 0.1 # location of lidar centre in b-frame primary axis, m
        lidar_yb = 0 # location of lidar centre in b-frame secondary axis, m 
        self.lidar = RangeAngleKinematics(lidar_xb, lidar_yb)

        # Wheel Configuration
        wheel_distance = 0.0815 #measured
        wheel_diameter = 0.065  #measured
        ...
        self.initial_wheelrate_right = 0.5 * np.pi  # Slower initial right wheel
        self.initial_wheelrate_left = 0.5 * np.pi   # Slower initial left wheel
        self.initial_command_duration = 2.0         # Run for 2 seconds
    ...
    def simulate_90_degree_turn(self):
        """Simulates a 90-degree turn using kinematic model"""
        # Turn parameters (90 degree left turn)
        turn_duration = 2.0  # seconds
        turn_rate = np.deg2rad(90)/turn_duration  # rad/s

        # Create control inputs: [v, ω] 
        # (v=0 for pure rotation, ω=π/4 rad/s for 2 seconds)
        t_list = np.linspace(0, turn_duration, 20)
        u_list = np.zeros((len(t_list), 2))
        u_list[:,1] = turn_rate  # Set angular rate

        # Initial state [x, y, θ, v, ω]
        state = Vector(5)
        state[:3] = [self.est_pose_northings_m, 
        ...
        for i in range(1, len(t_list)):
            ...
            # Get control input
            u = Vector(2)
            u[0] = u_list[i,0]  # Linear velocity
            u[1] = u_list[i,1]  # Angular velocity
            
            # Update state using kinematic model
            new_pose = rigid_body_kinematics(Vector(state[:3]), u, dt)
            
            # Update full state vector
            state[:3] = new_pose
            state[3:] = u
            X.append(copy.copy(state))
        ...
        # Plot trajectory
        plt.subplot(1,2,1)
        plot_path(X[:,:3], legend_flag=False)
        plt.title("Vehicle Trajectory")
        
        # Plot heading
        plt.subplot(1,2,2)
        plt.plot(t_list, np.rad2deg(X[:,2]))
        plt.xlabel("Time (s)")
        plt.ylabel("Heading (deg)")
        plt.title("Yaw Angle Evolution")
        
        plt.tight_layout()
        plt.show()

def run(self, time_to_run=-1):
    if hasattr(self, 'simulate_turn') and self.simulation:
        self.simulate_90_degree_turn()
    else:
        # Original run method implementation
        self.start_time = datetime.utcnow().timestamp()
        try:
            r = Rate(10.0)
            while True:
    ...
    def infinite_loop(self):
        ...
        if self.initialise_pose != True:
            ...
            if not hasattr(self, 'control_start_time'):
                self.control_start_time = datetime.utcnow().timestamp()
                return  # Skip control on first run

            if (datetime.utcnow().timestamp() - self.control_start_time) < self.initial_command_duration:
                # Send initial commands
                wheel_speed_msg = Vector3Stamped()
                wheel_speed_msg.vector.x = self.initial_wheelrate_right
                wheel_speed_msg.vector.y = self.initial_wheelrate_left
            ...
            if self.initialise_control == True:
                ...
                # print("Wheel rate right: ", q[0], "rad/s; Wheel rate left: ", q[1], "rad/s;")wheel_speed_msg = Vector3Stamped()
